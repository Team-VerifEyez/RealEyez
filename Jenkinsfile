                ...
                script {
                    sh '''
                        ...
                        if ! docker images --format "{{.Repository}}:{{.Tag}}" | grep -q '^joedhub/owasp_realeyez:latest$'; then
                            ...
                            docker pull joedhub/owasp_realeyez:latest &
                        else
                            echo "Image joedhub/owasp_realeyez:latest already exists locally. Skipping pull."
                        fi
                        
                        # Always pull the OWASP ZAP image in the background
